...
def test_course_startVideo(login_and_logout183):
    '''
    启动app并登陆
    点击创作
    点击课程进入播放页
...
# @allure.story('用例1')
@allure.severity('important')
def test_course_creat(login_and_logout183):
    '''
    启动app并登录账号
    验证是否成功进入发现页面
    点击打开创作方式选择页
    ...
    # with allure.step('点击菜单调出顶部菜单栏'):
    #     logger.info('点击菜单调出顶部菜单栏')
    #     # 点击菜单调出顶部菜单栏
    #     d(resourceId="com.codemao.nemo:id/menu").click()
    #
    # with allure.step('点击退出，退出创作页并等待5s'):
    #     logger.info('点击退出，退出创作页并等待5s')
    #     # 点击退出，退出创作页
...
# @allure.story('用例1')
@allure.severity('important')
def test_FullScreen(login_and_logout183):
    '''
    启动app并登陆
    点击选择创作方式
    点击课程进入播放页
...
# @allure.story('用例1')
@allure.severity('important')
def test_FullScreen_creat(login_and_logout183):
    '''
    启动app并登陆
    点击创作
    点击课程进入播放页
...
if __name__ == '__main__':
    ...
    pytest.main(["-s", "-q", "test_course.py", "--alluredir", path_xml])
